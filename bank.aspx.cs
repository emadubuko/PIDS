...
public partial class _Bank : Page
{
    ...
    protected void lbSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (objTrans.savePanel(bank, true) == false)
            {
    ...
    private void loadGrid(string sCondition)
    {
        try
        {
            ...
            objTrans.getNonTemplateGrid(gvListBank, sWhereClause);
        }
        catch (Exception ex)
        {
            DisplayError(objTrans.ErrorMessage + ex.Message);
    ...
    protected void gvList_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            GridViewRow dgi = gvListBank.SelectedRow;
            rec_id.Value = dgi.Cells[objTrans.getColumnIndexByName(gvListBank,"RecID")].Text;
            objTrans.getPanelByRecID(bank);
    ...
    protected void gvList_OnRowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            GridViewRow dgi = gvListBank.Rows[e.RowIndex];
            rec_id.Value = dgi.Cells[objTrans.getColumnIndexByName(gvListBank, "RecID")].Text;
            objTrans.deletePanelByRecID(bank);
            loadGrid("");
    ...
    protected void gvList_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        try
        {
            gvListBank.PageIndex = e.NewPageIndex;
    ...
    protected void ddlPaging_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            GridViewRow dgi;
            dgi = gvListBank.FooterRow;
            DropDownList _ddl = (DropDownList)dgi.FindControl("ddlPaging");
            gvListBank.PageSize = int.Parse(_ddl.SelectedValue);
            lbSearch_Click(sender, e);
            _ddl.SelectedValue = gvListBank.PageSize.ToString();
        }
        catch (Exception ex)
        {
            DisplayError(ex.Message);
